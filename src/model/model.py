...
class SarModel(BaseModel):
    def __init__(self,
        ...
        super().__init__()
        print(f"Initializing SarModel with {temporal_aggregator_type}...")
        num_cat_dict = load_yaml(num_cat_pkl_path)
        self.max_len = max_len
        self.hidden_size = hidden_size
        self.feature_embedders = nn.ModuleList([
            FeatureEmbedder(num_cat_dict,
                            ds,
                            emb_feat_dim,
                            hidden_size,
                            hidden_size_coeff,
    ...
    def forward(self, batch_idxs, seq_idxs, xs):
        ...
        xs: a list of data of given data_source, shape(5, len(given_data_source), emb_dim)
        """
        ...
        for bi, si, x in zip(batch_idxs, seq_idxs, xs):
            tmp = torch.zeros((batch_size,), device=bi.device).long()
            last_attn = tmp.scatter_reduce(dim=0, index=bi, src=si, reduce="amax")
            _x[bi, si] = x
            mask[bi, si] = 1

        # run temporal model
        x = self.temporal_aggregator(_x, mask, last_attn)
        tempagg_outs = x

        x = self.classifier(x).squeeze(-1)
        return x, tempagg_outs
